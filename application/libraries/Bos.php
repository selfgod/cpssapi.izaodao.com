...
include_once __DIR__ . '/BaiduBOS/BaiduBce.phar';
...
class Bos extends PHPUnit_Framework_TestCase
{
    private $client;
    private $client_mct;
    private $bucket;
    ...
    public function __construct()
    {
		...
		$BOS_TEST_CONFIG =
			array(
				'credentials' => array(
					'ak' => 'de89d2e06dd7443a9e4422d5b3fb4eea',
					'sk' => 'ebc53e595fc64858b2a152cea91304c7',
				),
	...
	/*初始化mct*/
    public function _construct_mct()
    {
		...
		$this->client_mct = new MediaClient($MEDIA_TEST_CONFIG);
    }
	
	
	/*创建job*/
    public function create_job($pipelineName, $sourceKey, $targetKey, $presetName)
    {	
		$this->_construct_mct();
		$this->client_mct->createSimpleJob($pipelineName, $sourceKey, $targetKey, $presetName);
		
    }
	

    /*列举buket*/
    public function listObjects($page=1 ,$t = 1, $MARKER='', $bucket = 'course-ware')//页数，当前页，文件名
    {
    	$options = array(
    			//BosOptions::MAX_KEYS=>3,
    			BosOptions::PREFIX=>"media/",
    			BosOptions::MARKER=>$MARKER,//文件名
    	...
    	if($page > 1 && $t != $page){
    		$t += 1;
    		$response = $this->listObjects($page,$t,$response->nextMarker);
    	}
    	return $response;
    }
    ...
    /*列举buket*/
    public function listObjects_tlmp3($page=1 ,$t = 1, $MARKER='')//页数，当前页，文件名
    {
    	$bucket = 'tlmp3';
    	$options = array(
    			//BosOptions::MAX_KEYS=>3,
    	...
    	if($page > 1 && $t != $page){
    		$t += 1;
    		$response = $this->listObjects_tlmp3($page,$t,$response->nextMarker);
    	}
    	return $response;
    }
	 ...
	 */
	public function generate_get_object_url($bucket, $object) {
		$this->bucket = $bucket;
		$this->key = $object;
		//generatePreSignedUrl with timestamp and expiration
		$signOptions = array(
			SignOptions::TIMESTAMP=>new \DateTime(),
			SignOptions::EXPIRATION_IN_SECONDS=>6000,
		);
	...
	/*调用*/
	public function check_file_exists($object ,$bucket = 'course-ware' ) {
		if($bucket == 'course-ware')
		$object = '/media/'.$object;
		try{
	...
	/*调用*/
	public function get_object_info($filename ,$bucket = 'course-ware' ) {
	...
	/*调用*/
	public function check_object_exists($object ,$bucket = 'course-ware' ) {
		try{
			$response = $this->client->getObjectMetadata($bucket, $object);
			return true;
		}catch(Exception $e){
...
//		$this->key = 'wei2.png';
		...
		$this->key = '/media/05yuecjst-0-N1.mp4';
		$this->bucket = 'course-mct';
		
		//generatePreSignedUrl with timestamp and expiration
		$signOptions = array(
			SignOptions::TIMESTAMP=>new \DateTime(),
			SignOptions::EXPIRATION_IN_SECONDS=>2000,
		);
		$url = $this->client->generatePreSignedUrl($this->bucket,
			$this->key,
			array(BosOptions::SIGN_OPTIONS => $signOptions)
